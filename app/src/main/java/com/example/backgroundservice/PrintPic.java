...
public class PrintPic {
    ...
    private static final PrintPic instance = new PrintPic();
    ...
    public static Bitmap mergeBitmapsVertically(Bitmap bitmap1, Bitmap bitmap2, Bitmap bitmap3, Bitmap bitmap4) {

        // Determine the merged bitmap size
        int mergedWidth = Math.max(bitmap1.getWidth(), bitmap2.getWidth());
        int mergedWidth2 = Math.max(bitmap3.getWidth(), mergedWidth);
        int mergedWidth3 = Math.max(bitmap4.getWidth(), mergedWidth2);
        int mergedHeight = bitmap1.getHeight() + bitmap2.getHeight() + bitmap3.getHeight() + bitmap4.getHeight();

        // Create a new bitmap for the merged image
        Bitmap mergedBitmap = Bitmap.createBitmap(mergedWidth3, mergedHeight, Bitmap.Config.ARGB_8888);

        // Create a canvas for drawing onto the merged bitmap
        Canvas canvas = new Canvas(mergedBitmap);

        // Draw the first bitmap at (0, 0)
        canvas.drawBitmap(bitmap1, 0, 0, null);

        // Draw the second bitmap below the first one
        int secondBitmapY = bitmap1.getHeight();
        canvas.drawBitmap(bitmap2, 0, secondBitmapY, null);

        // Draw the third bitmap below the second one
        int thirdBitmapY = secondBitmapY + bitmap2.getHeight();
        canvas.drawBitmap(bitmap3, 0, thirdBitmapY, null);

        // Draw the fourth bitmap below the third one
        int fourthBitmapY = thirdBitmapY + bitmap3.getHeight();
        canvas.drawBitmap(bitmap4, 0, fourthBitmapY, null);
